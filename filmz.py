...
def loadEmbed(csrf_name, movies, lnk, code):
    if(code == 2):
        ...
        datas = {'csrf_test_name': csrf_name,
                 'movie': movies}
        zhao = requests.post('https://www.filmz.cc/getlink',
                             data=datas, headers=headersPost)
        ...
        datas = {'csrf_test_name': csrf_name,
                 'episode': movies}
        zhao = requests.post('https://www.filmz.cc/getlinke',
                             data=datas, headers=headersPost)
        jsonLoad = simplejson.loads(zhao.content)
        tree2 = html.fromstring(jsonLoad['iframe'])
        hahay = tree2.xpath("//*/@src")
...
def getImdb(title):
    try:
        req = requests.get(
            'https://www.imdb.com/find?q={0}'.format(title), headers=HEADERS2)
        soup = BeautifulSoup(req.text, 'html.parser')
...
def getTmdb(title):
    try:
        req = requests.get(
            'https://www.themoviedb.org/search?query={0}'.format(title), headers=HEADERS2)
        soup = BeautifulSoup(req.text, 'html.parser')
...
try:
    if TYPE == 'episode':
        if not fileExist('episode.txt'):
            collectLinks('https://www.filmz.cc/series/page/', 'episode.txt', 2)
        ...
        for i, lnk in enumerate(arr):
            ...
            try:
                movies_code = tree.xpath(
                    "//button[contains(@class,'player') and contains(.,'openload')]/@onclick")
            ...
            try:
                movies_code = tree.xpath(
                    "//button[contains(@class,'player') and contains(.,'streamango')]/@onclick")
                if(len(movies_code) > 0):
                    movies_code = movies_code[0]
                ...
                continue
            sys.stdout.write('.')
            print ("%.3f " % (float(float(countah*100)/float(len(arr)))) + " % ")
            countah = countah+1
            # write file
            f = open('episode.csv', 'a')
            f.write(imdb+','+tmdb+','+(str)(season)+','+(str)
                    (episode)+','+'|'.join(srcs)+','+lnk+'\n')
            f.close()

    elif TYPE == 'movies':
        if not fileExist('movies.txt'):
            collectLinks('https://www.filmz.cc/films/page/', 'movies.txt', 2)

        # check csv
        tmp=list(set(readFile('movies.txt')))
       # tmp = set(readFile(['movies.txt']))
        ...
        if not fileExist('tvshows.txt'):
            collectLinks('https://www.filmz.cc/series/page/', 'tvshows.txt', 3)
        ...
        for i, lnk in enumerate(arr):
            ...
            for ep in eps:
                ...
                print("{0}/{1} {2}: Season {3} Episode {4}".format(i +
                                                                   1, (len)(arr), title, season, episode))
                sys.stdout.write('.')
